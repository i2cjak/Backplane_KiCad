...
std::optional<bool> API_HANDLER_BOARD::documentIsModified() const
{
    if( !board() )
        return std::nullopt;
...
void API_HANDLER_BOARD::onModified()
{
    if( board() )
        board()->SetModified();
}
        ...
        std::function<void( ItemStatus, google::protobuf::Any )> aItemHandler )
{
    ...
    for( const google::protobuf::Any& anyItem : aItems )
    {
        ...
        // Stable 10.0.6 has the regular BOARD_ITEM deserializer on PCB_GROUP;
        // the newer DeserializeGroup helper is not part of this branch.
        ...
        // BOARD_ITEM::FitsEnabledLayers was introduced after stable10.0.6.
        // Preserve the same guard using the stable layer-set interface.
        if( aCreate && item->Type() != PCB_GROUP_T
            && ( item->GetLayerSet() & board->GetEnabledLayers() ).none() )
        {
            status.set_code( ItemStatusCode::ISC_INVALID_DATA );
        ...
        if( aCreate )
        {
            ...
            else
            {
                ...
                // BOARD_COMMIT remaps footprint children to a parent-footprint
                // modification in the board editor.  Adding the child through
                // commit->Add() alone therefore records only the parent image
                // and drops the new child from the model.  Apply the same
                // parent-aware insertion used by the native editor.
        ...
        const HANDLER_CONTEXT<GetBoardStackup>& aCtx )
{
    ...
    google::protobuf::Any stackup;
    board()->GetStackupOrDefault().Serialize( stackup );
    stackup.UnpackTo( response.mutable_stackup() );
        ...
        const HANDLER_CONTEXT<GetActiveLayer>& aCtx )
{
    ...
    response.set_layer(
            ToProtoEnum<PCB_LAYER_ID, board::types::BoardLayer>(
                    m_frame ? static_cast<PCB_BASE_EDIT_FRAME*>( m_frame )->GetActiveLayer()
                            : board()->GetActiveLayer() ) );
        ...
        const HANDLER_CONTEXT<SetActiveLayer>& aCtx )
{
    ...
    if( m_frame )
        static_cast<PCB_BASE_EDIT_FRAME*>( m_frame )->SetActiveLayer( layer );
    else
