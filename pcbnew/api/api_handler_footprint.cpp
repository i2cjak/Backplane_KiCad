...
tl::expected<bool, ApiResponseStatus> API_HANDLER_FOOTPRINT::validateDocumentInternal( const DocumentSpecifier& aDocument ) const
{
    ...
    if( !target_fp.IsValid() && !target_fp.IsLegacy() )
        return false;
        ...
        const HANDLER_CONTEXT<OpenLibraryItem>& aCtx )
{
    ...
    if( !frame() )
    {
        ...
        e.set_error_message( "OpenLibraryItem requires an interactive Footprint Editor frame" );
        return tl::unexpected( e );
    }
    ...
    if( !footprintContext()->OpenFootprint( fpid ) )
    {
        ApiResponseStatus e;
        e.set_status( ApiStatusCode::AS_BAD_REQUEST );
        e.set_error_message( "could not activate footprint" );
        return tl::unexpected( e );
    }
        ...
        const HANDLER_CONTEXT<SaveCopyOfDocument>& aCtx )
{
    ...
    // A native .kicad_mod path is a document copy, rather than a library
    // item name.  Keep this path headless-safe and let PCB_IO write the
    // companion metadata beside the native file.
    wxFileName nativePath( pathStr );

    if( nativePath.GetExt().CmpNoCase( FILEEXT::KiCadFootprintFileExtension ) == 0 )
    {
        nativePath = wxFileName( project().AbsolutePath( pathStr ) );

        if( nativePath.FileExists() && !aCtx.Request.options().overwrite() )
        {
            ApiResponseStatus e;
            e.set_status( ApiStatusCode::AS_BAD_REQUEST );
            e.set_error_message( fmt::format( "save path '{}' exists and cannot be overwritten",
                                              nativePath.GetFullPath().ToStdString() ) );
            return tl::unexpected( e );
        }

        if( !nativePath.IsOk() || !nativePath.IsDirWritable() )
        {
            ApiResponseStatus e;
            e.set_status( ApiStatusCode::AS_BAD_REQUEST );
        ...
        if( !footprintContext()->SavePcbCopy( nativePath.GetFullPath(),
                                               aCtx.Request.options().include_project(), true ) )
        {
            ApiResponseStatus e;
            e.set_status( ApiStatusCode::AS_BAD_REQUEST );
            e.set_error_message( "failed to save footprint copy" );
            return tl::unexpected( e );
        }
        ...
        const HANDLER_CONTEXT<RevertDocument>& aCtx )
{
    ...
    if( !footprintContext()->RevertFootprint() )
    {
        ApiResponseStatus e;
        e.set_status( ApiStatusCode::AS_BAD_REQUEST );
        e.set_error_message( "failed to revert footprint" );
        return tl::unexpected( e );
    }
